...
models = train %>%
  model('Seasonal Naive' = fable::SNAIVE(diff ~ lag(365)),              # Baseline for comparison (seasonal naive model)
        ...
        'ARMA(1,1)' = ARIMA(diff ~ 0 + pdq(1, 0, 1) + PDQ(0, 0, 0)),
        'ARMA(1,2)' = ARIMA(diff ~ 0 + pdq(1, 0, 2) + PDQ(0, 0, 0)),
        'ARMA(2,1)' = ARIMA(diff ~ 0 + pdq(2, 0, 1) + PDQ(0, 0, 0)),
        'ARMA(0,1)' = ARIMA(diff ~ 0 + pdq(0, 0, 1) + PDQ(0, 0, 0)),
        'ARMA(1,0)' = ARIMA(diff ~ 0 + pdq(1, 0, 0) + PDQ(0, 0, 0)),
        'SARMA(1,2)(0,2)[7]' = ARIMA(diff ~ 0 + pdq(1, 0, 2)))
...
ggplot(box_ljung_table, aes(x = h, y = lb_pvalue)) +
  geom_point(pch = 1) +
  geom_hline(yintercept = 0.05, color = '#437cb5', linetype = 'dashed') +
...
```{r roots, echo = F, fig.align='center'}
# checking for roots
models %>%
  select('SARMA(1,2)(0,2)[7]') %>%
  gg_arma()+
...
Fitted values of the $SARMA(1,2)(0,2)_{7}$ model are transformed to the original scale in order to obtain a fitted plot as seen in Figure 8. The fit of the $SARMA(1,2)(0,2)_{7}$ model (blue) is plotted on top of the number of unique visits (black). 
\newline
...
ggplot(dta, aes(x = Date)) +
  ...
  labs(x = 'Date', y = 'Unique visits',
       title = 'Fitted SARMA(1,2)(0,2)[7] Model',
...
arima_forecasts = models %>%
  select(c("SARMA(1,2)(0,2)[7]","Seasonal Naive"))%>%
  forecast(h = '341 days')
...
Table 2 and Figure 9 summarize the performance results of our models on the test set. All models outperformed the seasonal naive baseline as measured by MAE and RMSE. Overall, the $SARMA(1,2)(0,2)_{7}$ model performed the best, having the lowest MAE and RMSE.
...
final_model = web %>%
  model('SARMA(1,2)(0,2)[7]' = ARIMA(diff ~ 0 + pdq(1,0,2)))
...
ggplot(data = final_data, aes(x = Date)) +
  ...
  labs(x = 'Date', y = 'Unique Visits', title = paste(h,'-Day SARMA(1,2)(0,2)[7] Forecast', sep = ''))

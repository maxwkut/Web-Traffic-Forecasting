...
Both ACF and PACF show a slow decay indicating that an ARMA(2,2) model could be appropriate for the seasonally adjusted series. An auto regressive moving average of order p and q (ARMA(p,q)) model is defined in Eq(3):
...
The fitted values of the ARIMA(2,2) model are plotted against the actual values of the seasonally differenced series $\triangledown x_t^*$ in Figure 7. 
\newline

```{r Fig7, echo=F, message=FALSE, results='hide', warning=FALSE, eval=F}
...
```{r include=F, eval=F, echo=F, eval=F}

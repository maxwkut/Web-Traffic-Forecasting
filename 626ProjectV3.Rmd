...
models = train %>%
  model('Seasonal Naive' = fable::SNAIVE(diff ~ lag(365)),                        # Baseline for comparison (seasonal naive model)
        'ARIMA(0,0,1)(0,1,0)[7]' = ARIMA(diff ~ 1 + pdq(0, 0, 1) + PDQ(0, 1, 0, period = 7)),
        'ARIMA(1,0,0)(0,1,0)[7]' = ARIMA(diff ~ 1 + pdq(1, 0, 0) + PDQ(0, 1, 0, period = 7)),
        'ARIMA(1,0,1)(0,1,0)[7]' = ARIMA(diff ~ 1 + pdq(1, 0, 1) + PDQ(0, 1, 0, period = 7)),
        'ARIMA(1,0,2)(0,1,0)[7]' = ARIMA(diff ~ 1 + pdq(1, 0, 2) + PDQ(0, 1, 0, period = 7)),
        'ARIMA(1,0,2)(0,1,1)[7]' = ARIMA(diff ~ 1 + pdq(1, 0, 2) + PDQ(0, 1, 1, period = 7)),
        'ARIMA(1,0,2)(1,1,1)[7]' = ARIMA(diff ~ 1 + pdq(1, 0, 2) + PDQ(1, 1, 1, period = 7)),
        'ARIMA(1,0,2)(0,1,2)[7]' = ARIMA(diff ~ 1 + pdq(1, 0, 2) + PDQ(0, 1, 2, period = 7)),
        'ARIMA(1,0,2)(1,1,2)[7]' = ARIMA(diff ~ 1 + pdq(1, 0, 2) + PDQ(1, 1, 2, period = 7)),
        'ARIMA(1,0,2)(2,1,2)[7]' = ARIMA(diff ~ 1 + pdq(1, 0, 2) + PDQ(2, 1, 2, period = 7)))

# Retrieves the parameter estimates and hypothesis tests (p-values) for a specific model
models %>%
  select('ARIMA(1,0,2)(1,1,2)[7]') %>%
  report()
...
models %>%
  ...
  arrange(AIC) %>%
  kable(col.names = c('Model', 'Innov. Std. Error','Log-Likelihood','AIC','AICc','BIC'),
        digits = 2, align = c('l', 'c', 'c', 'c', 'c', 'c'))
```

We decided to choose the $SARMA(1,2)(0,2)_{7}$ model as it performed best in terms of AIC and BIC. Its parameters are estimated via maximum likelihood and are displayed in (4).
...
models %>%
  select('SARMA(1,2)(0,2)[7]') %>%
  report()
```

\begin{equation}
  \hat{x}_t = 0.94_{(0.02)}x_{t-1}+ \omega_t - 0.34_{(0.03)}\omega_{t-1} - 0.26_{(0.03)}\omega_{t-2}-0.65_{(0.03)}\omega_{t-7}-0.18_{(0.03)}\omega_{t-14}
\end{equation}  
...
Figure 7 plots the inverse AR and MA roots of our SARMA model. A causal invertible model should have all the roots outside the unit circle. Equivalently, the inverse roots should lie inside the unit circle (shown in red). Hence, there dosn't appear to be any parameter redundancy.
...
Fitted values of the $ARIMA(1,0,2)(1,1,2)_{7}$ model are transformed to the original scale in order to obtain a fitted plot as seen in Figure 8. The fit of the $ARIMA(1,0,2)(1,1,2)_{7}$ model (blue) is plotted on top of the number of unique visits (black). 
\newline
...
arima_mdl_details = models %>%
  report() %>%
  filter(.model == 'ARIMA(1,0,2)(1,1,2)[7]')
...
ggplot(fit_data, aes(x = Date)) +
  ...
  labs(x = 'Date', y = expression(y[t]),
       title = 'ARIMA(1,0,2)(1,1,2)[7] Model',
...
ggplot(dta, aes(x = Date)) +
  ...
  labs(x = 'Date', y = 'Unique visits',
       title = 'Back-transformed ARIMA(1,0,2)(1,1,2)[7] Model',
       caption = 'Fitted model shown by black line and the actual values of the training set are shown in red.')
...
where $\epsilon_t$ is modeled as a non-seasonal ARIMA process.
...
Table 2 and Figure 9 summarize the performance results of our models on the test set. All models outperformed the seasonal naive baseline as measured by MAE and RMSE. Overall, the $ARIMA(1,0,2)(1,1,2)_{7}$ model performed the best, having the lowest MAE and RMSE.
...
accuracy(arima_forecasts, web) %>%
  ...
  kableExtra::pack_rows("Baseline", 4,4 )
```
...
accuracy(arima_forecasts, web) %>%
  ...
  theme(legend.position = "none", text=element_text(family = 'cm',size=14))+
  scale_x_discrete(labels=c('SARMA(1,2)(0,2)[7]'="SARMA", 
...
```{r echo = F}
# Plot the forecasts
ggplot(data = arima_data %>% filter(.model == 'ARIMA(1,0,2)(1,1,2)[7]'), aes(x = Date)) +
  ...
  labs(x = 'Date', y = 'Unique Visits', title = 'ARIMA(1,0,2)(1,1,2)[7]')
```  
...
final_model = web %>%
  model('ARIMA(1,0,2)(1,1,2)[7]' = ARIMA(diff ~ 0 + pdq(1,0,2) + PDQ(1,1,2, period = 7)))
...
ggplot(data = final_data, aes(x = Date)) +
  ...
  labs(x = 'Date', y = 'Unique Visits', title = paste(h,'-Day ARIMA(1,0,2)(1,1,2)[7] Forecast', sep = ''))

final_data %>%
  mutate(PI_range = back_upper95 - back_lower95) %>%
  filter(Date >= '2020-08-20') %>%
  select(back_lower95, back_pred, back_upper95, PI_range)
...
final_data %>%
  ...
  kable(col.names = c('Date', 'Lower 95% CI', 'Forecast','Upper 95% CI'),
        digits = 0, caption = paste(h, '-Day ARIMA(1,0,2)(1,1,2)[7]Forecast.', sep = ''))
```
\newpage

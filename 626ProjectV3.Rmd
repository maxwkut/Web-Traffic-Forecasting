...
One challenge with the unique visits, however, is the complex seasonality. Multiple seasonal patterns exist within the time series, and the family of SARIMA(p,d,q)(P,D,Q)[s] models only allow for a single seasonal difference. In an attempt to handle the complex seasonality, we performed a two-step seasonal differencing approach by taking the annual difference of the time series, then taking the weekly difference of the transformed time series from the previous step. This can be more clearly shown as

\begin{equation}
  x_t^* = \triangledown_7 \triangledown_{365} x_t = (1 - B^7)(1 - B^{365})x_t = (x_t - x_{t-7}) - (x_{t-365}-x_{t-372})
\end{equation}

where B is the backshift operator. Time plots of the aforementioned transformation steps series at different stages are displayed in Figure 4. The differenced series appears to be stationary with a constant mean and variance.
\newline
...
We opted to fit a range of ARMA(p,q) models with small orders, with the final model selected based the forecasting accuracy of the models on a test or hold-out set since our primary goal for this analysis is forecasting. The model selection criterion for the fitted ARMA models is shown in the table below. 
...
models = train %>%
  model('Seasonal Naive' = fable::SNAIVE(diff ~ lag(365)),              # Baseline for comparison (seasonal naive model)
        ...
        'SARMA(1,2)(2,2)[7]' = ARIMA(diff ~ 0 + pdq(1, 0, 2)))

models %>%
  select('SARMA(1,2)(0,0)[7]') %>%
  report()

models2 = train %>%
  model('test' = ARIMA(annual_diff ~ 1 + pdq(1,0,2) + PDQ(0, 0, 1, period = 7)),
        'test2' = ARIMA(annual_diff ~ 1 + pdq(1,0,2) + PDQ(0, 1, 2, period = 7)),
        'test3' = ARIMA(annual_diff ~ 1 + pdq(1,0,2) + PDQ(1, 1, 2, period = 7)))
...
\begin{equation}
  \hat{x}_t = 0.7164_{(0.0414)}x_{t-1}+ \omega_t - 0.1953_{(0.0489)}\omega_{t-1} - 0.089_{(0.0325)}\omega_{t-2}
\end{equation}  
...
ar_roots = round(unnest(roots[,1], cols = 'ar_roots'),2)
ma_roots = round(unnest(roots[,2], cols = 'ma_roots'),2)

roots = unnest(roots, cols = c(ar_roots, ma_roots))

for (i in 2:nrow(roots)) {
  roots$ar_roots[i] = NA
}

kable(roots,
      col.names = c('AR Roots', 'MA Roots'),
      align = c('c','c'), caption = 'Roots of polynomials', digits = 2)
```
...
The forecast accuracy for the ARMA(1,2) model shows the best performance on the test set, 
